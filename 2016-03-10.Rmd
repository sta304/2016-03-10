...
## variance of $r$ - II { .build .smaller }

$\left(\bar y - R \bar x\right)$ is actually a pretty simple object. Let (NOTE: changed this to capital $R_i$.):
...
(This isn't practically useful yet...we don't know $R$!)
...
where $s^2_r$ is $s^2_R$ with $r$ used in place of $R$ and $\bar r$ used in place of $\bar R$, and use $\bar x$ if $\mu_x$ is unknown. 

## salary example



Let's estimate the population salary rate of increase and place the usual bound on the error of estimation $\left(B = 2\sqrt{\hat V(r)}\right)$. Here are the details. 
...
(See the file `6_10.xlsx` on github with these slides for a spreadsheet solution to the problem.)
...
$$\hat\tau_y = r\tau_x = \frac{\sum y_i}{\sum x_i}\tau_x = \frac{0.246}{4.35}(1800) = 101.79$$
